...
　^ も同様ですが、*counts* に安易に大きな数値を指定すると、関数の戻り値によってはコンピューターのリソースを消費し尽くす点に注意してください。戻り値の大きさは、関数の実行結果の要素数を *counts* でべき乗すると確かめられます。
```javascript
console.log(
	strings("[:echo(3, ','): 'い', 'ろ', 'は']")
	// この関数の実行結果は、い、ろ、は、の三つの文字のすべての組み合わせ、全 27 通りになる。
);
```
